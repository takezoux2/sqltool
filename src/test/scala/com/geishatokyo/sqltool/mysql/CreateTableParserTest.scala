...
class CreateTableParserTest extends Specification {
  ...
  "Variable sql type" should{
    ...
    val sqlGeneratedByMySQLServer3 =
    ...
    "parse create table sql3 generated by MySQL server" in{

      val tableDef = CreateTableParser(sqlGeneratedByMySQLServer3)

      tableDef.name must_== "musicinfo"

      tableDef.column("id") must_== Column("id","bigint(20)",List(NotNull(),AutoIncrement()))
      tableDef.primaryKey must_== Some(PrimaryKey(List("id")))
      tableDef.uniqueIndexes(0).name === "KEY_MusicInfo_musicHash"
